    ...
    <>
      <div
        className={useClasses(
          "pl-6 py-5 float-right absolute right-0 bottom-3 cursor-pointer z-10"
        )}
...
export const SendBtn = (props) => {
  return (
    <>
      <div
        className={useClasses(
          "pl-6 py-5 float-right absolute right-0 bottom-28 cursor-pointer z-10"
        )}
        onClick={() => props.onClick()}
      >
        <div
...
export const Editor: FC<any> = (props) => {
  ...
  const [post, setPost] = useState<any>(
    props.post || props.type === "post" ? new PostModel() : new PageModel()
  );
  useEffect(() => {
  ...
  return (
    <div>
      ...
      <div>
        <div
          className={useClasses(
            "pl-6 py-5 float-right absolute right-0 bottom-16 cursor-pointer z-10"
          )}
          onClick={() => {
          ...
          <Drawer.Content className="drawer-content">
            ...
            {props.type === "post" && (
              <div>
                ...
                <Toggle
                  checked={
                    post.copyright === true || post.copyright === false
                      ? post.copyright
                      : true
                  }
                  // defaultChecked={true}
                ...
                <Radio.Group
                  initialValue={
                    post.hide === true || post.hide === false
                      ? post.hide === true
                        ? 1
                        : 0
                      : 0
                ...
                >
                  <Radio value={1}>
                    ...
                    <Radio.Description>
                      <Text>
                        将会显示在全部接口中,{" "}
                        <Text b>如果要隐藏于RSS接口中, 请在下方选择选项</Text>
                      </Text>
                    </Radio.Description>
                  </Radio>
                  <Radio value={0}>
                    ...
                    <Radio.Description>
                      <Text>
                        将会从全部接口中隐藏,{" "}
                        <Text b>包括 RSS 接口，届时下方选项将会失效</Text>
                      </Text>
                    </Radio.Description>
                  </Radio>
                </Radio.Group>
                ...
                <Input.Password
                  ...
                  defaultValue={
                    post?.password === null ? undefined : post?.password
                  }
                  onChange={(e) => {
                ...
                <Collapse title="温馨提示">
                  <Spacer h={0.5} />
                  <Text small i>
                    若已设置密码将<Text b>显示已被加密过</Text>的密码
                  </Text>
                  <Spacer h={0.5} />
                  <Text small i>
                    但输入密码您仅需输入<Text b>明文</Text>密码,
                  </Text>
                  <Spacer h={0.5} />
                  <Text small i>
                    删除密码删除文本框内值即可
                ...
                >
                  <Radio value={1}>
                    ...
                    <Radio.Description>
                      <Text>
                        将会显示在 RSS 接口中,{" "}
                        <Text b>如果要隐藏于全部接口中, 请选择上方选项</Text>
                      </Text>
                    </Radio.Description>
                  </Radio>
                  <Radio value={0}>
                    ...
                    <Radio.Description>
                      <Text>
                        将会从 RSS 接口中隐藏,{" "}
                        <Text b>若你已选择了上方隐藏选项，选项将会失效</Text>
                      </Text>
                    </Radio.Description>
                  </Radio>
                </Radio.Group>
                    ...
                    post.tags.map((tag, index) => {
                      return (
                        ...
                        >
                          {tag}
                          <span
                            className={useClasses(tagStyles.editTagAction)}
                            onClick={() => {
                              setPost({
                                ...post,
                  ...
                  <AutoComplete
                    ...
                    onKeyDown={(e) => {
                      if (e.keyCode === 13) {
                        ...
                        document.getElementById("tagInput")!.style.display =
                          "none";
      ...
      <BackBtn
        onClick={() => {
          ...
          if (JSON.stringify(post) !== JSON.stringify(props.post)) {
            window.confirm("编辑内容未保存，确定返回吗？").valueOf() &&
              window.history.back();
          } else {
            window.history.back();
          }
