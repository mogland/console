...
export const setStorage = (key: string, value: any, expire = 10000) => {
  ...
  if (isNaN(expire) || expire < 1)
    throw new Error("expire must be a number greater than 0");
...
export const getStorage = (key: any) => {
  if (
    !window[config.type].getItem(`${config.prefix}-${key}`) ||
    JSON.stringify(window[config.type].getItem(`${config.prefix}-${key}`)) ===
      "null"
  ) {
    // 如果没有值
    return null;
  }
  const data = config.isEncrypt
    ? JSON.parse(
        decrypt(window[config.type].getItem(`${config.prefix}-${key}`))
      )
    : JSON.parse(window[config.type].getItem(`${config.prefix}-${key}`));
  if (data.expire < Date.now() - data.time) {
    // 如果过期了
...
export const hasStorage = (key: any) => {
  ...
  const arr = getAllStorage().filter((item) => {
    return item.key === key;
  });
