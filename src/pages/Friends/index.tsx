...
import { Clear, Delete, Edit, AddOne, Detection, SendEmail, CheckSmall, CloseSmall, Redo, Rss } from "@icon-park/react"
...
import { Input, Textarea } from "../Write/Input"
...
export const FriendsPage: BasicPage = () => {
  ...
  const EditModal = () => {
    const [item, setItem] = useState<any>(friends.find((i) => i.id === select[0]) || {})
    return (
      <>
        <Modal
              ...
              friends?.map((item, index) => {
                return (
                  <TableItem
                    ...
                    aria-label={item.id}
                    className={select.includes(item.id) && postStyles.select || ""}
                  >
                    ...
                    <TableItemValue>{item.auto_check ? "已互链" : "疑似不存在"}</TableItemValue>
                  </TableItem>
                )
              })
            }
  ...
  return (
    <>
      ...
      <div className={clsx(postStyles.container, "loading", !loading && "loaded")}>
        <Title>
            ...
            <div>
              {
                select.length && <button
                  ...
                  onClick={() => {
                    setSelect([])
                    const items = document.querySelectorAll(`.${postStyles.select}`)
                    items.forEach((item) => {
              ...
              }
              {
                select.length && <button
                  className={postStyles.button}
                  onClick={(e) => {
                    if (e.currentTarget.classList.contains(postStyles.confrim)) {
                      // select.forEach((item) => {
                      //   apiClient(`/post/${item}`, {
                      //     method: "DELETE",
                      //   })
                      // })
                      setSelect([])
                      document.querySelectorAll(`.${postStyles.select}`).forEach((item) => {
                        item.remove()
                      })
                    } else {
              ...
              }
              {
                ...
                ><CheckSmall /></button> || null
              }
              {
                (select.length && tab == 0) && <button
                  className={postStyles.button}
                  onClick={(e) => {
                    if (e.currentTarget.classList.contains(postStyles.confrim)) {
                      select.forEach((item) => {
                ...
                ><CloseSmall /></button> || null
              }
              {
                select.length && <button
                  className={postStyles.button}
                  onClick={() => {
                    window.open(`mailto:${select.map((item) => {
                      return friends.find((i) => i.id === item)?.email
                    }).join(",")}`)
                  }}
                ><SendEmail /></button> || null
              }
              {
                select.length && <button
                  className={postStyles.button}
                  onClick={() => {
                    ...
                    select.forEach((item) => {
                      apiClient(`/friends/${item}/check`).then((res) => {
                        ...
                        Twindow({
                          title: `互链检查 - ${friends.find((i) => i.id === item)?.name}`,
                          text: res.data ? "站点已互链" : "检查验证链接后疑似不存在，建议自行前往站点检查",
                        })
                      })
                    })
                  }}
                    ...
                    <button className={postStyles.button}
                      onClick={() => {
                        ...
                        apiClient("/friends/alive").then((res) => {
                          Twindow({
                            title: "检测结果",
                            text: `检测到 ${res.data.filter((item) => item.is_alive).length} 个有效链接`,
                          })
                          res.data.forEach((item) => {
                            const target = document.querySelector(`[aria-label="${item.id}"]`)
                            if (target) {
                              if (item.is_alive) {
        ...
        >
          <Tab.List className={styles.tab}>
            <Tab className={({ selected }) => clsx(styles.tabItem, selected && styles.tabItemActive)}>已通过</Tab>
            <Tab className={({ selected }) => clsx(styles.tabItem, selected && styles.tabItemActive)}>待审核</Tab>
            <Tab className={({ selected }) => clsx(styles.tabItem, selected && styles.tabItemActive)}>垃圾友链</Tab>
            <Tab className={({ selected }) => clsx(styles.tabItem, selected && styles.tabItemActive)}>回收站</Tab>
          </Tab.List>
          <Tab.Panels>
