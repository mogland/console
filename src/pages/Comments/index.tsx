    ...
    await apiClient.get('/comment', null, [{ key: "page", value: nowPage }, { key: "size", value: 5 }, { key: "status", value: Number(nowTab) - 1 }]).then(res => {
      ...
      for (let index of Object.keys(data)) {
        content.push({
          ...
          ref_type: data[index].ref_type === "Post" && '文章' || data[index].ref_type === 'Page' && '页面',
  ...
  return (
    <NxPage title={"Comments"}>
      <Dashboards.Container className="lg:grid flex flex-col" gridTemplateColumns='1fr'>
        <Dashboards.Area className={useClasses("overflow-x-hidden")} style={{ overflow: "auto", height: "75vh" }}>
          {
            !loading ? (<>
