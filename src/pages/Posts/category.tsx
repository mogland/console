...
import { Button, Input, Modal, Popover, Select, Spacer, Table, Text, useClasses, useModal } from "@geist-ui/core";
...
export const Category: BasicPage = () => {
  ...
  const [selectCategory, setSelectCategory] = useState<{ from: string, to: string }>({ from: '', to: '' })
  const [editCategory, setEditCategory] = useState<any>({})

  const { visible, setVisible, bindings } = useModal()
  const { visible: addVisible, setVisible: setAddVisible, bindings: addBindings } = useModal()
  const { visible: mergeVisible, setVisible: setMergeVisible, bindings: mergeBindings } = useModal()
  const { visible: editVisible, setVisible: setEditVisible, bindings: editBindings } = useModal()

  const request = async () => {
    await apiClient.get('/categories', null, [{ key: "type", value: "Category" }]).then(res => {
      setCategories(res.data)
    })
    await apiClient.get('/categories', null, [{ key: "type", value: "Tag" }]).then(res => {
      setTags(res.data)
    })
  }
  useMount(() => {
  ...
  const renderCategoryAction = (value, rowData, index) => {
    ...
    const removeContent = () => {
      return (
        <div style={{ padding: 20, paddingBottom: 0 }}>
          <Text h5 b>是否要删除分类 {categories[index].name} ？</Text>
          <Text p b>将会把分类下全部文章一同删除</Text>
          <Button auto scale={1 / 3} font="12px" style={{ margin: 10 }}
            onClick={async () => {
              await apiClient.delete('/categories', [categories[index].id]).then(res => {
    ...
    return (
      <>
        <Button auto scale={1 / 3} font="12px" onClick={() => {
          setEditVisible(true)
        ...
        <Spacer w={.5} />
        <Popover content={removeContent} visible={deleteVisible} onVisibleChange={changeHandler}>
          <Button type="error" auto scale={1 / 3} font="12px">Remove</Button>
        </Popover>
        <Spacer w={.5} />
        <Button type="success" auto scale={1 / 3} font="12px" onClick={() => {
        ...
        <Modal.Action
          onClick={async () => {
            if (selectCategory.from && selectCategory.to) {
              await apiClient.post('/categories/merge', null, null, {
                type: "Category",
                ...selectCategory
              }).then(res => {
  ...
  const AddCategoryModal = () => {
    return (
      <Modal {...addBindings}>
        ...
        <Modal.Action passive onClick={() => setAddVisible(false)}>取消</Modal.Action>
        <Modal.Action
          onClick={async () => {
            const name = document.getElementById('add-category-name') as HTMLInputElement
            const slug = document.getElementById('add-category-slug') as HTMLInputElement
  ...
  const EditCategoryModal = () => {
    return (
      <Modal {...editBindings}>
        ...
        <Modal.Content >
          <Input placeholder="分类名称" id="edit-category-name" width={"100%"} initialValue={editCategory.name} />
          <Spacer />
          <Input placeholder="分类路径" id="edit-category-slug" width={"100%"} initialValue={editCategory.slug} />
        </Modal.Content>
        <Modal.Action passive onClick={() => setEditVisible(false)}>取消</Modal.Action>
        <Modal.Action
          onClick={async () => {
            const name = document.getElementById('edit-category-name') as HTMLInputElement
            const slug = document.getElementById('edit-category-slug') as HTMLInputElement
            ...
            !loading && (
              <>
                <Table data={categories}>
                  ...
                  <Table.Column label="操作" prop="action" render={renderCategoryAction} />
                </Table>
              </>
            )
          }
            ...
            !loading && (
              <>
                <Table data={tags}>
                  ...
                  <Table.Column label="操作" prop="action" render={renderTagAction} />
                </Table>
              </>
            )
          }
