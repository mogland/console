          ...
          >
            {
              data.data.map((item, index) => {
                return <TableItem
                  header={["TITLE", "DATE", "SITE", "AUTHOR"]}
                  className={clsx(styles.tableItem, "item", pageStyles.tableItem)}
                  onClick={() => {
                    window.open(item.link)
                  }}
                  key={index}>
                  <TableItemValue> {item.title} </TableItemValue>
                  <TableItemValue> {new Date(item.published_date).toLocaleDateString()} </TableItemValue>
                  <TableItemValue> {item.link.match(/\/\/(.*?)\//)?.[1]} </TableItemValue>
